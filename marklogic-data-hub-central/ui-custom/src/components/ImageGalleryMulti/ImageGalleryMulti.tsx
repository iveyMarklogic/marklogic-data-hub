...
const ImageGalleryMulti: React.FC<Props> = (props) => {
  ...
  const ButtonGroup = ({next, previous, goToSlide, ...rest}) => {
    ...
    return (
      <>
        <a className="navigation-button prev" onClick={() => previous()} >
          <ChevronLeft size={60} className={currentSlide === 0 ? 'disable' : ''} />
        </a>
        <a className="navigation-button next" onClick={() => next()} >
